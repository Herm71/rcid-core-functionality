...
-   **`npm test` now fires automatically on commit.** It runs `phpcs` over staged `.php` and `wp-scripts format` over staged JS/JSON/YAML/MD, and husky invokes it from `.husky/pre-commit`. The hook installs itself through the `prepare` script, so it is live after `npm install` — but only in a working copy where that has been run, and `git commit --no-verify` skips it. Treat it as fast feedback; [lint.yml](.github/workflows/lint.yml) is the gate that cannot be bypassed. There is still no test suite, so "tests pass" remains the wrong phrase for a green run.
-   **CI lints every pull request** via [lint.yml](.github/workflows/lint.yml): `composer validate` and `composer lint` on PHP 8.0 (the `Requires PHP` floor), plus `npm run lint:pkg-json` and a formatting check on Node 20. The formatting check runs `npm run format` and fails if the tree moved, because `wp-scripts format` has no `--check` mode — so a formatting-only failure means "run `npm run format` and commit the result", not "something is broken".
...
**The version lives in two places, and both are now automated.** `commit-and-tag-version` bumps `package.json`, `package-lock.json`, and — via the `bumpFiles` entry in `package.json` — the `Version:` header in [plugin.php](plugin.php), which is what WordPress reads for its update checks. That header is rewritten by [wp-plugin-version-updater.js](wp-plugin-version-updater.js), a small `readVersion`/`writeVersion` module; the built-in `plain-text` updater can't be used because it expects a file containing nothing but a version string. Do _not_ bump `plugin.php` by hand anymore (releases through v1.2.2 did, which is why older commits look that way) — a hand-edit that lands in the same commit range just makes the two disagree.
...
-   **[post-types.php](lib/functions/post-types.php)** — registers `projects`, `press`, and `testimonials` CPTs, all with `show_in_rest => true` (required for the block editor, and for the block theme to query them) and `has_archive => true`. A fourth, `team_member`, is defined but its `add_action('init', ...)` is commented out; leave that intact unless asked to enable it.

    Each CPT has its own `rcid_register_*_post_type()` function, but `init` hooks a single aggregator, `rcid_register_post_types()`, and [plugin.php](plugin.php)'s activation hook calls that same function. **Add a new CPT to the aggregator, not to `init` directly** — if the two paths diverge, the new archive silently 404s on a fresh activation. `rcid_post_type_slugs()` is the single source of truth for which slugs exist.
...
-   **[security-headers.php](lib/functions/security-headers.php)** — a single `wp_headers` filter setting CSP, `X-Frame-Options`, `Permissions-Policy`, etc., skipped when `is_admin()`. The most fragile thing in this repo: several past releases were CSP hotfixes after a third-party script got blocked. Any new external script, font, or iframe source must be added to the matching directive or it will silently fail on the live site.
    ...
    **Violations go nowhere unless `RCID_CSP_REPORT_URI` is defined in `wp-config.php`** — set it to the collector URL and the report-only policy gains `report-uri` and `report-to` directives plus a matching `Reporting-Endpoints` header. It is read from a constant rather than hard-coded because collector URLs embed an account identifier that should not sit in a public repo. Unset, or set to anything that is not a valid `https` URL, the reporting directives and the header are all omitted rather than emitted broken — a bad URL would make every page load fire a pointless cross-origin POST. `RCID_CSP_REPORT_TO_URI` is an optional second constant for collectors like report-uri.com that issue a different URL for the Reporting API than for the legacy `report-uri` directive; it defaults to the same endpoint.

    Both directives are sent on purpose: `report-uri` is deprecated but is what Safari and older browsers honour, while Chromium uses `report-to` and ignores `report-uri` when both are present.

-   **[gtm.php](lib/functions/gtm.php)** — Google Tag Manager (container `GTM-5K5NV59S`) injected via `wp_head` and `wp_body_open`, both at priority `-1` so the snippet lands as high in the document as possible. That priority is intentional (v1.2.2); don't "clean it up" to the default. GA is intentionally _not_ here — it was removed in v1.2.1 in favor of GTM.
...
-   **[uninstall.php](uninstall.php)** — runs on plugin _deletion_, not deactivation. **The uninstall policy is that content outlives the plugin**: `projects`, `press` and `testimonials` posts, terms and meta are never touched, because surviving a theme rewrite is the entire point of those post types. Deleting the plugin only stops them being registered — the content is hidden, not destroyed. Anything added here that deletes a post needs a deliberate decision and an opt-in, and the `WP_UNINSTALL_PLUGIN` guard at the top must stay. What it _does_ remove is plugin-update-checker's bookkeeping: the `external_updates-…` site option, the `puc_manual_check_errors-…` site transient, and the `puc_cron_check_updates-…` cron event. Those names derive from the slug passed to `buildUpdateChecker()`; change the slug and they must change too.
...
-   It is deliberately _not_ gated on `wp_is_auto_update_enabled_for_type('plugin')`. That function only reports whether unattended background auto-updates are on; the update notice and the manual Update Now button both need the check to run regardless.
